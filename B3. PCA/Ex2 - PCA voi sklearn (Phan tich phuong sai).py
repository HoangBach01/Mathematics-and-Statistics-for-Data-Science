...
data = pd.read_excel("./Data/B3/Student_12f.xls")
print(data.head(), "\n")
...
for k in range(1, 13):
    ...
    # Test - BEGIN
    print(
        "- Chuyển vị của ma trận vectơ riêng P_T",
...
print(
    "   Muốn phương sai tích lũy >= %.0f%%" % percent,
    "thì k >= %d" % k,
    " (k = %d" % k,
    "--> %.0f%%)" % var,
)
...
A = np.array([0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.99])
for x in A:
    percent = x * 100
    pca = PCA(x)
